...
bash "$ROOT_DIR/electron-frontend/build-appimage.sh" "$VERSION_NUMBER"

echo
echo "[2/3] Building Qt AppImage..."
bash "$ROOT_DIR/qt_frontend/build-appimage.sh" --version "$VERSION_NUMBER"

echo
echo "[3/3] Building minimal AppImage..."
bash "$ROOT_DIR/minimal_frontend/build-appimage.sh" --version "$VERSION_NUMBER"

...
// Adapted from niklashenning/qt-toast (MIT, © 2024 Niklas Henning).
// Vendored into this app and modified to render INLINE: each toast is a child
// widget of a host widget (positioned relative to the host's rect) rather than a
// top-level window — top-level self-positioning isn't possible on Wayland, which
// would otherwise leave every toast centred on screen. CSS is inlined (no asset
// files) and icons are off.
...
public:
    ...
    static void reset();
    // Re-anchor all on-screen toasts to the host's current rect (call on host resize).

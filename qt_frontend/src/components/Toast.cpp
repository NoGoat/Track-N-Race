    ...
    : QWidget(parent)
{
    ...
    // Apply stylesheet (inlined from the original toast.css so there's no asset
    // file to ship/locate at runtime).
    setStyleSheet(
...
void Toast::setStayOnTop(bool enabled)
{
    ...
    // Inline toast: it's a child widget, NOT a top-level window. Setting any
    // Qt::Window/Qt::Tool flag here would promote it to its own window — which on
    // Wayland the compositor centres on screen. Stacking above sibling page
    // content is handled by raise() in show() instead, so we deliberately set no
    // window flags here.
}
...
void Toast::show()
{
    ...
    if (s_maximumOnScreen > s_currentlyShown.size())
    {
        ...
        // Fade in. raise() keeps the inline toast above its sibling page content.
        QWidget::show();
        raise();
...
void Toast::setupUI()
{
    ...
    // Resize window
    QWidget::setFixedSize(totalWidth, totalHeight);
...
QPoint Toast::calculatePosition()
{
    ...
    // Inline toasts are children of the host widget, so positions are computed in
    // the host's local coordinate space (origin top-left) rather than on a screen.
    const int hostW = m_parent ? m_parent->width()  : 0;
    const int hostH = m_parent ? m_parent->height() : 0;
    ...
    if (s_position == ToastPosition::BOTTOM_RIGHT)
    {
        x = hostW - m_notification->width() - s_offsetX;
        y = hostH - m_notification->height() - s_offsetY - offsetY;
    }
    ...
    {
        x = hostW / 2 - m_notification->width() / 2;
        y = hostH - m_notification->height() - s_offsetY - offsetY;
    }
    else if (s_position == ToastPosition::TOP_RIGHT)
    {
    ...
    {
        x = hostW / 2 - m_notification->width() / 2;
        y = hostH / 2 - m_notification->height() / 2 + offsetY;
    }

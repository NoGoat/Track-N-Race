...
// Per-car record sizes — F1 24 specific
...
const DAMAGE_SIZE = 42
const TYRE_SET_SIZE = 10

// Packet 13 — MotionEx offsets (same as F1 25; extra fields only appended after)
...
// -----------------------------------------------------------------------
// Packet 6: Car Telemetry  (identical to F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 13: MotionEx — front/rear aero heights (same offsets as F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 0: Motion — g-forces  (identical to F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 0: Motion — all-car world positions (x, z only) (identical to F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 2: Lap (player only)  (identical to F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 2: Timing tower (all 22 cars)  (identical to F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 7: Car Status (player)  (identical to F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 7: All-car status  (identical to F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 10: Car Damage — F1 24 (no tyreBlisters, DAMAGE_SIZE = 42)
// -----------------------------------------------------------------------
function damage(data: Buffer, hdr: PacketHeader): Row[] {
  ...
  o += 8  // skip 4B tyre_damage% + 4B detached (brakes damage)
  // F1 24 has NO tyreBlisters[4] here — next fields are wing damage directly
  const wingFL      = b[o++]
  const wingFR      = b[o++]
  const wingRear    = b[o++]
  const floorDmg    = b[o++]
  ...
  return [{
    ...
    // No blisters in F1 24 — emit zeros so DamageRow type is satisfied
...
// -----------------------------------------------------------------------
// Packet 4: Participants — F1 24 (PART_SIZE = 61, name = 48 chars, no livery)
// -----------------------------------------------------------------------
function participants(data: Buffer, _hdr: PacketHeader): Row[] {
  ...
  for (let i = 0; i < 22; i++) {
    ...
    // F1 24: name is 48 bytes (vs 32 in F1 25)
    const nameStart = o
...
// -----------------------------------------------------------------------
// Packet 3: Event codes — F1 24 differences:
//   RTMT: only vehicleIdx (no reason byte)
//   SGSV: only vehicleIdx (no stopTime float)
//   DRSD: no payload at all
// -----------------------------------------------------------------------
function event(data: Buffer, hdr: PacketHeader): Row[] {
  ...
  switch (code) {
    ...
    // F1 24: RTMT has only vehicleIdx (no reason byte like F1 25)
    case 'RTMT':
    case 'RCWN': {
    ...
    // F1 24: DTSV has only vehicleIdx
    case 'DTSV': {
      if (data.length < o + 1) return []
      return [{ ...base, car_idx: data[o] }]
    }
    // F1 24: SGSV has only vehicleIdx (no stopTime float like F1 25)
    case 'SGSV': {
      if (data.length < o + 1) return []
      return [{ ...base, car_idx: data[o] }]
    }
...
// -----------------------------------------------------------------------
// Packet 12: TyreSets  (identical to F1 25)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// Packet 11: SessionHistory  (identical to F1 25)
// -----------------------------------------------------------------------

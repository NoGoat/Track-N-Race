...
std::vector<std::string> F1_25::ParsePacket(const uint8_t* data, int length, const PacketHeader& hdr, const std::string& timestamp) {
    ...
    switch (hdr.packetId) {
        ...
        case PID_CAR_STATUS: {
            ...
            AllStatusCar sc = ParseStatusF125(data, base);
            ...
            if (length >= HEADER_SIZE + 22 * statusSize) {
                ...
                ar.cars.resize(22);
                for (int i = 0; i < 22; ++i) {
                    ar.cars[i] = ParseStatusF125(data, HEADER_SIZE + i * statusSize);
                    ar.cars[i].idx = i;
                }
                buf.clear();

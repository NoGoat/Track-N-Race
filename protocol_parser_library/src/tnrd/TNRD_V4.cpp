...
bool splitRows(std::string_view plain,uint8_t expectedType,uint64_t sequence,std::vector<V4TimedRow>& out,uint32_t expectedCount,std::string* errorOut,const std::vector<float>* inferredTimes=nullptr,bool filter=false,float from=0,float to=0,bool latestOnly=false,float* firstOut=nullptr,float* lastOut=nullptr){
    ...
    while(pos<plain.size()){
        ...
        if(nl>pos){auto line=plain.substr(pos,nl-pos);const uint8_t actual=rowType(line);if(actual!=expectedType){fail(errorOut,"V4 chunk contains the wrong row type");return false;}float time=scanTime(line);if(time<0&&inferredTimes&&count<inferredTimes->size())time=(*inferredTimes)[count];if(std::isfinite(time)){first=std::min(first,time);last=std::max(last,time);}if(!filter||(time>=from&&time<=to)){if(latestOnly){if(!haveBest||time>bestTime){bestTime=time;bestLine=line;bestOffset=(uint32_t)pos;haveBest=true;}}else out.push_back({time,actual,sequence,std::string(line),(uint32_t)pos});}++count;}
        if(nl==plain.size())break;pos=nl+1;
    }
    if(count!=expectedCount){fail(errorOut,"V4 chunk row count mismatch");return false;}if(latestOnly&&haveBest)out.push_back({bestTime,expectedType,sequence,std::string(bestLine),bestOffset});if(firstOut)*firstOut=first;if(lastOut)*lastOut=last;return true;
}
...
void mergeRowGroups(std::vector<std::vector<V4TimedRow>>& groups,std::vector<V4TimedRow>& out){
...
struct TnrdV4Archive::Impl {
    ...
    bool loadChunk(size_t index,std::FILE* reader,std::shared_ptr<std::string>& result,std::string* errorOut,std::shared_ptr<ParsedRows>* parsedOut=nullptr){
        ...
        if(!reader)loaded.error="could not open a parallel V4 reader handle";
            ...
            if(!readAt(reader,c.offset-CHUNK_PREFIX_SIZE,input.data(),input.size())){loaded.error="truncated V4 chunk at lap "+std::to_string(c.lapNumber);loaded.permanentError=true;}
            else if(get32(prefix)!=CHUNK_MAGIC||get32(prefix+4)!=c.lapNumber||get16(prefix+8)!=c.rowType||get16(prefix+10)!=c.flags||get64(prefix+12)!=c.compressedSize||get64(prefix+20)!=c.uncompressedSize||get32(prefix+28)!=c.rowCount){loaded.error="V4 chunk prefix does not match its directory entry";loaded.permanentError=true;}
            else{
    ...
    bool splitChunk(size_t index,std::FILE* reader,std::vector<V4TimedRow>& out,std::string* errorOut,std::shared_ptr<std::string> plain={},bool filter=false,float from=0,float to=0,bool latestOnly=false){
...
int TnrdV4Archive::lapAt(float time)const{const auto it=std::upper_bound(impl_->laps.begin(),impl_->laps.end(),time,[](float value,const V4LapInfo& lap){return value<lap.startSessionTime;});return it==impl_->laps.begin()?0:(int)std::prev(it)->lapNumber;}

void TnrdV4Archive::chunkIndicesForLap(uint32_t lap,V4RowTypeMask mask,std::vector<size_t>& out)const{
    out.clear();auto it=impl_->chunkIndex.lower_bound({lap,0});for(;it!=impl_->chunkIndex.end()&&it->first.first==lap;++it)if(mask&v4TypeBit((uint8_t)it->first.second))out.insert(out.end(),it->second.begin(),it->second.end());
}
bool TnrdV4Archive::chunkTimeBounds(size_t index,float& firstOut,float& lastOut)const{
...
void TnrdV4Archive::prefetchChunk(size_t index){
...
void TnrdV4Archive::cancelPrefetch(){impl_->executor.cancelPrefetch();}
...
bool TnrdV4Archive::open(const std::string& path,HeaderRow& header,std::string* errorOut){
    ...
    if(validatedFooter!=fo||validatedSummary.size()!=activeSummarySize||validatedLaps.size()!=(size_t)lc*LAP_ENTRY_SIZE||validatedChunks.size()!=(size_t)cc*CHUNK_ENTRY_SIZE){fail(errorOut,"could not retain V4 control tables");close();return false;}
    std::string summaryJson=std::move(validatedSummary);if((activeSummaryOffset||activeSummarySize)&&glz::read_json(impl_->summary,summaryJson)){fail(errorOut,"invalid V4 control summary");close();return false;}
...
bool TnrdV4Archive::rowsForChunks(const std::vector<size_t>& indices,
                                  std::vector<std::vector<V4TimedRow>>& out,
                                  std::string* errorOut){
    out.clear();for(size_t index:indices)if(index>=impl_->chunks.size()){fail(errorOut,"V4 chunk index is out of bounds");return false;}
...
bool TnrdV4Archive::rowsForLap(uint32_t lap,V4RowTypeMask mask,std::vector<V4TimedRow>& out,std::string* errorOut){
    ...
    std::vector<std::vector<V4TimedRow>> groups;groups.reserve(results.size());for(auto& result:results){if(!result.ok){fail(errorOut,result.error);return false;}groups.push_back(std::move(result.rows));}mergeRowGroups(groups,out);return true;
}
bool TnrdV4Archive::rowsForLapRange(uint32_t lap,float from,float to,V4RowTypeMask mask,std::vector<V4TimedRow>& out,std::string* errorOut,const IndexedCancelCheck& cancelled){
    std::vector<size_t> selected;auto it=impl_->chunkIndex.lower_bound({lap,0});for(;it!=impl_->chunkIndex.end()&&it->first.first==lap;++it)if(mask&v4TypeBit((uint8_t)it->first.second))selected.insert(selected.end(),it->second.begin(),it->second.end());
...
bool TnrdV4Archive::rowsForRange(float from,float to,V4RowTypeMask mask,std::vector<V4TimedRow>& out,std::string* errorOut,const IndexedCancelCheck& cancelled){
...
bool TnrdV4Archive::latestRows(float at,const std::vector<uint8_t>& types,std::vector<V4TimedRow>& out,std::string* errorOut,const IndexedCancelCheck& cancelled){

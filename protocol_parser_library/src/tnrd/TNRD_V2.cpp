...
public:
    ...
    bool flushRecoverable() override {
        ...
        return std::fflush(file_) == 0 || fail("fflush failed for TNRD V2 output");
    }
    bool finish() override {
        ...
        if (file_) {
            if (std::fclose(file_) != 0 && ok) ok = fail("fclose failed for TNRD V2 output");
            file_ = nullptr;
        }
...
private:
    ...
    bool pump(ZSTD_inBuffer& input, ZSTD_EndDirective directive, bool allowEmpty,
        ...
        if (!check(remaining, "TNRD V2 compression failed")) return false;
        if (output.pos > 0 && std::fwrite(output.dst, 1, output.pos, file_) != output.pos)
            return fail("write failed for TNRD V2 output");
        if (!allowEmpty && output.pos == 0 && input.pos == input.size && remaining != 0)
            return fail("TNRD V2 compression made no progress");
...
bool load(const std::string& path, LoadResult& result, std::string& error) {
    ...
    result.tempPath = (std::filesystem::temp_directory_path() /
        ("tracknrace_temp_v2_" + std::to_string(
            std::chrono::steady_clock::now().time_since_epoch().count()) + ".tmp")).string();
    if (!decompressTnrd(path, result.tempPath, TnrdFormat::ZstdV2,
                        &result.partial, &error)) {
        if (error.empty()) error = "The TNRD V2 recording could not be decompressed.";
        discard(result);
        return false;
    }
    std::FILE* file = openTnrdFile(result.tempPath, "rb");
    if (!file) {
        error = "The decompressed TNRD V2 file could not be opened.";
        discard(result);
        return false;
    }
    std::string line;
    ...
    if (glz::read<kPartialRead>(result.header, line)) {
        error = "The TNRD V2 header is missing or contains invalid JSON.";
        discard(result);
        return false;
    }
    if (result.header.magic != "TNRD_V2" || !result.header.compression ||
        *result.header.compression != "zstd") {
        error = "The TNRD V2 header does not match its Zstandard container.";
        discard(result);
        return false;
    }
    return true;

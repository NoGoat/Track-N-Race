...
std::vector<uint8_t> makeHeader(uint64_t metaOff,uint64_t metaSize,uint64_t lapOff,uint32_t lapCount,
    ...
    std::vector<uint8_t>b;b.insert(b.end(),MAGIC.begin(),MAGIC.end());put16(b,5);put16(b,(uint16_t)HEADER_SIZE);put32(b,3);
...
struct TnrdV5Writer::Impl {
    ...
    bool writeSnapshot(bool includeRowIndex,std::string* errorOut){
        ...
        if(laps.size()>MAX_LAPS||chunks.size()>MAX_CHUNKS||branches.size()>MAX_BRANCHES){fail(errorOut,"V5 control table exceeds its format limit");return false;}
        ...
        if(!writeAll(file,branchBytes.data(),branchBytes.size())){fail(errorOut,"failed to append V5 branch table");return false;}
        ...
        put32(footer,FOOTER_MAGIC);put16(footer,5);put16(footer,FOOTER_SIZE);
...
struct TnrdV5Archive::Impl {
    ...
    std::FILE* file{};uint64_t fileSize{};HeaderRow header;std::vector<V5LapInfo> laps;std::vector<V5ChunkInfo> chunks;std::vector<BranchCut> branches;V5ControlSummary summary;
...
bool TnrdV5Archive::open(const std::string& path,HeaderRow& header,std::string* errorOut){
    ...
    if(headerCrcOk&&(get32(h.data()+12)!=3||get32(h.data()+124)!=0)){fail(errorOut,"V5 file uses unsupported feature flags");close();return false;}
    ...
    if(headerCrcOk&&(get32(h.data()+44)!=LAP_ENTRY_SIZE||get32(h.data()+60)!=CHUNK_ENTRY_SIZE||get32(h.data()+116)!=BRANCH_ENTRY_SIZE)){fail(errorOut,"V5 header table sizes are invalid");close();return false;}
    ...
    if(validatedFooter!=fo||validatedSummary.size()!=activeSummarySize||validatedBranches.size()!=(size_t)bc*BRANCH_ENTRY_SIZE||validatedLaps.size()!=(size_t)lc*LAP_ENTRY_SIZE||validatedChunks.size()!=(size_t)cc*CHUNK_ENTRY_SIZE||validatedRows.size()!=rs){fail(errorOut,"could not retain V5 control tables");close();return false;}

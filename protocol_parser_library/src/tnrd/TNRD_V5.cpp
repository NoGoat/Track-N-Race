...
struct TnrdV5Writer::Impl {
    ...
    bool writeSnapshot(std::string* errorOut){
        ...
        for(size_t index:order){auto& c=chunks[index];c.prefixMaxLastTime=prefixMax[c.type]=std::max(prefixMax[c.type],c.lastTime);c.rowIndexOffset=nextRowIndexOffset;c.rowIndexCount=(uint32_t)c.rowIndex.size();put32(dir,c.lap);put16(dir,c.type);put16(dir,c.flags);put64(dir,c.offset);put64(dir,c.compressed);put64(dir,c.plain);put32(dir,c.rows);put32(dir,c.crc);put64(dir,c.sequence);putFloat(dir,c.firstTime);putFloat(dir,c.lastTime);putFloat(dir,c.prefixMaxLastTime);putFloat(dir,c.minDistance);putFloat(dir,c.maxDistance);put64(dir,c.rowIndexOffset);put32(dir,c.rowIndexCount);put16(dir,c.rowIndexStride);put16(dir,c.rowIndexEntrySize);put32(dir,0);for(const auto& row:c.rowIndex){putFloat(rowIndexBytes,row.firstTime);putFloat(rowIndexBytes,row.lastTime);put32(rowIndexBytes,row.offset);put32(rowIndexBytes,row.length);put32(rowIndexBytes,row.ordinal);put16(rowIndexBytes,row.count);put16(rowIndexBytes,row.reserved);}nextRowIndexOffset+=c.rowIndex.size()*ROW_INDEX_ENTRY_SIZE;}
...
struct TnrdV5Archive::Impl {
    ...
    struct ChunkMeta{float first{},last{},prefixMax{},minDistance{},maxDistance{};uint64_t rowIndexOffset{};uint32_t rowIndexCount{};uint16_t rowIndexStride{},rowIndexEntrySize{};std::vector<RowSpan> rows;};
    ...
    std::map<std::pair<uint32_t,uint16_t>,std::vector<size_t>> chunkIndex;std::vector<ChunkMeta> chunkMetadata;
    ...
    void clear(){
        executor.stop();if(file)std::fclose(file);file=nullptr;fileSize=0;path.clear();laps.clear();chunks.clear();summary={};chunkIndex.clear();chunkMetadata.clear();chunkTimeBounds.clear();
...
bool TnrdV5Archive::open(const std::string& path,HeaderRow& header,std::string* errorOut){
    ...
    std::vector<uint8_t> lapBytes=std::move(validatedLaps),dir=std::move(validatedChunks),rowIndexBytes=std::move(validatedRows);
    ...
    const uint64_t maxCompressed=ZSTD_compressBound((size_t)MAX_CHUNK_PLAIN);std::vector<std::pair<uint64_t,uint64_t>> payloadRanges;std::set<std::tuple<uint32_t,uint16_t,uint64_t>> keys;std::array<float,15> familyPrefix;familyPrefix.fill(-std::numeric_limits<float>::infinity());
    for(uint32_t i=0;i<cc;++i){
        ...
        if((c.lapNumber&&!lapKeys.count(c.lapNumber))||c.rowType>14||c.flags||!c.sequence||!c.compressedSize||!c.uncompressedSize||!c.rowCount||c.uncompressedSize>MAX_CHUNK_PLAIN||c.compressedSize>maxCompressed||c.compressedSize>SIZE_MAX||payloadOverflow||!rangeOk(impl_->fileSize,c.offset,c.compressedSize)||c.offset<CHUNK_PREFIX_SIZE||controlOverlap||!keys.emplace(c.lapNumber,c.rowType,c.sequence).second||!std::isfinite(meta.first)||!std::isfinite(meta.last)||meta.last<meta.first||!std::isfinite(meta.prefixMax)||meta.prefixMax!=expectedPrefix||!distancesValid||!meta.rowIndexCount||meta.rowIndexStride!=expectedStride||meta.rowIndexEntrySize!=ROW_INDEX_ENTRY_SIZE||get32(p+84)!=0||indexOverflow||meta.rowIndexOffset<ro||!rangeOk(ro+rs,meta.rowIndexOffset,indexBytes)){
            fail(errorOut,"invalid or duplicate V5 chunk entry");close();return false;
        }
        familyPrefix[c.rowType]=expectedPrefix;meta.rows.reserve(meta.rowIndexCount);const size_t relative=(size_t)(meta.rowIndexOffset-ro);uint32_t previousEnd=0,totalIndexedRows=0;float previousTime=-std::numeric_limits<float>::infinity(),indexedFirst=std::numeric_limits<float>::infinity(),indexedLast=-std::numeric_limits<float>::infinity();for(uint32_t row=0;row<meta.rowIndexCount;++row){const uint8_t*q=rowIndexBytes.data()+relative+(size_t)row*ROW_INDEX_ENTRY_SIZE;const float firstTime=getFloat(q),lastTime=getFloat(q+4);const uint32_t rowOffset=get32(q+8),rowLength=get32(q+12),ordinal=get32(q+16);const uint16_t count=get16(q+20),reserved=get16(q+22);if(!std::isfinite(firstTime)||!std::isfinite(lastTime)||lastTime<firstTime||firstTime<previousTime||!rowLength||rowOffset>c.uncompressedSize||rowLength>c.uncompressedSize-rowOffset||ordinal!=totalIndexedRows||!count||count>meta.rowIndexStride||reserved||(row&&rowOffset<=previousEnd)){fail(errorOut,"invalid V5 row index");close();return false;}previousTime=lastTime;previousEnd=rowOffset+rowLength-1;totalIndexedRows+=count;indexedFirst=std::min(indexedFirst,firstTime);indexedLast=std::max(indexedLast,lastTime);meta.rows.push_back({rowOffset,rowLength,firstTime,lastTime,ordinal,count});}if(totalIndexedRows!=c.rowCount||indexedFirst!=meta.first||indexedLast!=meta.last){fail(errorOut,"V5 row index time bounds mismatch");close();return false;}
    ...
    std::sort(payloadRanges.begin(),payloadRanges.end());for(size_t i=1;i<payloadRanges.size();++i)if(payloadRanges[i].first<payloadRanges[i-1].second){fail(errorOut,"V5 chunk payload ranges overlap");close();return false;}impl_->chunkTimeBounds.resize(impl_->chunks.size());for(size_t i=0;i<impl_->chunks.size();++i){impl_->chunkTimeBounds[i]={impl_->chunkMetadata[i].first,impl_->chunkMetadata[i].last,true};impl_->chunkIndex[{impl_->chunks[i].lapNumber,impl_->chunks[i].rowType}].push_back(i);}for(auto&[key,indices]:impl_->chunkIndex)std::sort(indices.begin(),indices.end(),[&](size_t a,size_t b){return impl_->chunks[a].sequence<impl_->chunks[b].sequence;});
    return true;
}
...
bool TnrdV5Archive::rowsForLapRange(uint32_t lap,float from,float to,V5RowTypeMask mask,std::vector<V5TimedRow>& out,std::string* errorOut,const IndexedCancelCheck& cancelled){
    std::vector<size_t> selected;auto it=impl_->chunkIndex.lower_bound({lap,0});for(;it!=impl_->chunkIndex.end()&&it->first.first==lap;++it)if(mask&v5TypeBit((uint8_t)it->first.second))for(size_t index:it->second){const auto bounds=impl_->bounds(index);if(!bounds.known||!(bounds.last<from||bounds.first>to))selected.push_back(index);}
...
bool TnrdV5Archive::latestRows(float at,const std::vector<uint8_t>& types,std::vector<V5TimedRow>& out,std::string* errorOut,const IndexedCancelCheck& cancelled){
    if(cancelled&&cancelled()){fail(errorOut,"indexed read cancelled");return false;}std::vector<size_t> selected;selected.reserve(types.size());
    for(uint8_t type:types){size_t bestChunk=SIZE_MAX;float bestTime=-std::numeric_limits<float>::infinity();uint64_t bestSequence=0;for(size_t index=0;index<impl_->chunks.size();++index){const auto& chunk=impl_->chunks[index];if(chunk.rowType!=type)continue;const auto& rows=impl_->chunkMetadata[index].rows;const auto it=std::upper_bound(rows.begin(),rows.end(),at,[](float value,const Impl::RowSpan& row){return value<row.firstTime;});if(it==rows.begin())continue;const auto& candidate=*std::prev(it);const float time=std::min(at,candidate.lastTime);if(bestChunk==SIZE_MAX||time>bestTime||(time==bestTime&&chunk.sequence>bestSequence)){bestChunk=index;bestTime=time;bestSequence=chunk.sequence;}}if(bestChunk!=SIZE_MAX)selected.push_back(bestChunk);}
    auto results=impl_->runChunkJobs(selected,[state=impl_.get(),at](std::FILE* file,size_t index){Impl::RowsResult result;if(!state->splitChunk(index,file,result.rows,&result.error,{},true,-std::numeric_limits<float>::infinity(),at,true))result.ok=false;return result;},cancelled);if(results.size()!=selected.size()){fail(errorOut,"indexed read cancelled");return false;}out.clear();for(auto& result:results){if(!result.ok){fail(errorOut,result.error);return false;}for(auto& row:result.rows)out.push_back(std::move(row));}sortRows(out);return true;
}

...
struct TnrdV4Writer::Impl {
    ...
    std::FILE* file{};uint64_t metadataOffset{HEADER_SIZE+METADATA_PREFIX_SIZE},metadataSize{};uint32_t currentLap{};float lastTime{};bool haveTime{};
...
bool TnrdV4Writer::append(const std::vector<V4SourceRow>& rows,std::string* errorOut){
    ...
    for(const auto&r:rows){std::string_view source=r.line;while(!source.empty()&&(source.back()=='\n'||source.back()=='\r'))source.remove_suffix(1);if(source.empty())continue;const float t=r.sessionTime>=0?r.sessionTime:scanTime(source);const std::string stored=withSessionTime(source,t);const std::string_view line=stored;const uint8_t type=rowType(line);if(t>=0){impl_->lastTime=std::max(impl_->lastTime,t);if(!impl_->haveTime){impl_->summary.startSessionTime=t;impl_->haveTime=true;}impl_->summary.totalSessionTime=std::max(impl_->summary.totalSessionTime,t);}
        if(type==4){const int n=scanInt(line,"\"lap_num\":",(int)impl_->currentLap);if(n>0&&(uint32_t)n!=impl_->currentLap){if(impl_->currentLap)impl_->laps[impl_->currentLap].end=t;impl_->currentLap=(uint32_t)n;float start=t;const int curMs=scanInt(line,"\"current_lap_ms\":",0);if(curMs>0)start=t-curMs/1000.0f;auto& lap=impl_->laps[impl_->currentLap];lap.num=impl_->currentLap;lap.start=start;lap.end=t;const int lastMs=scanInt(line,"\"last_lap_ms\":",0);auto prev=impl_->laps.find(impl_->currentLap-1);if(prev!=impl_->laps.end()&&lastMs>0){prev->second.timeMs=(uint32_t)lastMs;prev->second.flags|=1;}}}
...
struct TnrdV4Archive::Impl {
    ...
    size_t cacheLimit{DEFAULT_CACHE_BYTES},cacheBytes{};uint64_t decompressions{};std::list<size_t> lru;std::unordered_map<size_t,CacheEntry> cache;

    void clear(){if(file)std::fclose(file);file=nullptr;fileSize=0;laps.clear();chunks.clear();summary={};cache.clear();lru.clear();cacheBytes=0;decompressions=0;}
    bool loadChunk(size_t index,std::shared_ptr<std::string>& result,std::string* errorOut){
        ...
        const auto& c=chunks[index];std::array<uint8_t,CHUNK_PREFIX_SIZE> prefix{};if(!readAt(file,c.offset-CHUNK_PREFIX_SIZE,prefix.data(),prefix.size())||get32(prefix.data())!=CHUNK_MAGIC||get32(prefix.data()+4)!=c.lapNumber||get16(prefix.data()+8)!=c.rowType||get16(prefix.data()+10)!=c.flags||get64(prefix.data()+12)!=c.compressedSize||get64(prefix.data()+20)!=c.uncompressedSize||get32(prefix.data()+28)!=c.rowCount){fail(errorOut,"V4 chunk prefix does not match its directory entry");return false;}std::vector<uint8_t> compressed((size_t)c.compressedSize);auto plain=std::make_shared<std::string>((size_t)c.uncompressedSize,'\0');
        if(!readAt(file,c.offset,compressed.data(),compressed.size())){fail(errorOut,"truncated V4 chunk at lap "+std::to_string(c.lapNumber));return false;}
        const size_t got=ZSTD_decompress(plain->data(),plain->size(),compressed.data(),compressed.size());++decompressions;
        if(ZSTD_isError(got)||got!=c.uncompressedSize||(uint32_t)::crc32(0,(const Bytef*)plain->data(),(uInt)plain->size())!=c.checksum){fail(errorOut,"V4 chunk integrity check failed for lap "+std::to_string(c.lapNumber)+" type "+std::to_string(c.rowType));return false;}
        result=plain;if(plain->size()>cacheLimit)return true;while(cacheBytes+plain->size()>cacheLimit&&!lru.empty()){const size_t old=lru.back();lru.pop_back();auto it=cache.find(old);cacheBytes-=it->second.plain->size();cache.erase(it);}lru.push_front(index);cache[index]={plain,lru.begin()};cacheBytes+=plain->size();return true;
    }
    bool splitChunk(size_t index,std::vector<V4TimedRow>& out,std::string* errorOut){
        const auto& c=chunks[index];std::shared_ptr<std::string> plain;if(!loadChunk(index,plain,errorOut))return false;
        ...
        if(scanTime(first)>=0)return splitRows(*plain,(uint8_t)c.rowType,c.sequence,out,c.rowCount,errorOut);
        ...
        return splitRows(*plain,(uint8_t)c.rowType,c.sequence,out,c.rowCount,errorOut,&inferred);
    }
};
...
bool TnrdV4Archive::open(const std::string& path,HeaderRow& header,std::string* errorOut){
    close();impl_->file=openTnrdFile(path,"rb");if(!impl_->file){fail(errorOut,"could not open V4 file");return false;}if(!seekEnd(impl_->file)){fail(errorOut,"could not size V4 file");close();return false;}impl_->fileSize=tell(impl_->file);
    ...
    const uint64_t maxCompressed=ZSTD_compressBound((size_t)MAX_CHUNK_PLAIN);std::vector<std::pair<uint64_t,uint64_t>> payloadRanges;std::set<std::tuple<uint32_t,uint16_t,uint64_t>> keys;for(uint32_t i=0;i<cc;++i){const uint8_t*p=dir.data()+i*CHUNK_ENTRY_SIZE;V4ChunkInfo c{get32(p),get16(p+4),get16(p+6),get64(p+8),get64(p+16),get64(p+24),get32(p+32),get32(p+36),get64(p+40)};const bool payloadOverflow=c.compressedSize>UINT64_MAX-CHUNK_PREFIX_SIZE;const uint64_t payloadStart=c.offset>=CHUNK_PREFIX_SIZE?c.offset-CHUNK_PREFIX_SIZE:0,payloadSize=payloadOverflow?0:c.compressedSize+CHUNK_PREFIX_SIZE;const bool controlOverlap=!payloadOverflow&&(overlaps(payloadStart,payloadSize,0,HEADER_SIZE)||overlaps(payloadStart,payloadSize,HEADER_SIZE,METADATA_PREFIX_SIZE)||overlaps(payloadStart,payloadSize,mo,ms)||(activeSummarySize&&overlaps(payloadStart,payloadSize,activeSummaryOffset,activeSummarySize))||overlaps(payloadStart,payloadSize,lo,(uint64_t)lc*LAP_ENTRY_SIZE)||overlaps(payloadStart,payloadSize,co,(uint64_t)cc*CHUNK_ENTRY_SIZE)||overlaps(payloadStart,payloadSize,fo,FOOTER_SIZE));if((c.lapNumber&&!lapKeys.count(c.lapNumber))||c.rowType>14||c.flags||!c.sequence||!c.compressedSize||!c.uncompressedSize||!c.rowCount||c.uncompressedSize>MAX_CHUNK_PLAIN||c.compressedSize>maxCompressed||c.compressedSize>SIZE_MAX||payloadOverflow||!rangeOk(impl_->fileSize,c.offset,c.compressedSize)||c.offset<CHUNK_PREFIX_SIZE||controlOverlap||!keys.emplace(c.lapNumber,c.rowType,c.sequence).second){fail(errorOut,"invalid or duplicate V4 chunk entry");close();return false;}payloadRanges.emplace_back(payloadStart,payloadStart+payloadSize);impl_->chunks.push_back(c);}std::sort(payloadRanges.begin(),payloadRanges.end());for(size_t i=1;i<payloadRanges.size();++i)if(payloadRanges[i].first<payloadRanges[i-1].second){fail(errorOut,"V4 chunk payload ranges overlap");close();return false;}
    return true;
}

bool TnrdV4Archive::rowsForLap(uint32_t lap,V4RowTypeMask mask,std::vector<V4TimedRow>& out,std::string* errorOut){out.clear();for(size_t i=0;i<impl_->chunks.size();++i){const auto&c=impl_->chunks[i];if(c.lapNumber!=lap||!(mask&v4TypeBit((uint8_t)c.rowType)))continue;if(!impl_->splitChunk(i,out,errorOut))return false;}sortRows(out);return true;}
bool TnrdV4Archive::rowsForRange(float from,float to,V4RowTypeMask mask,std::vector<V4TimedRow>& out,std::string* errorOut){out.clear();for(size_t i=0;i<impl_->chunks.size();++i){const auto&c=impl_->chunks[i];if(!(mask&v4TypeBit((uint8_t)c.rowType)))continue;auto lap=std::find_if(impl_->laps.begin(),impl_->laps.end(),[&](const auto&l){return l.lapNumber==c.lapNumber;});if(c.lapNumber&&lap!=impl_->laps.end()&&(lap->endSessionTime<from||lap->startSessionTime>to))continue;std::vector<V4TimedRow> rows;if(!impl_->splitChunk(i,rows,errorOut))return false;for(auto&r:rows)if(r.sessionTime>=from&&r.sessionTime<=to)out.push_back(std::move(r));}sortRows(out);return true;}
bool TnrdV4Archive::latestRows(float at,const std::vector<uint8_t>& types,std::vector<V4TimedRow>& out,std::string* errorOut){
    out.clear();const int targetLap=lapAt(at);
    for(uint8_t type:types){bool found=false;V4TimedRow best;uint32_t foundLap=0;
        std::vector<size_t> candidates;for(size_t i=0;i<impl_->chunks.size();++i)if(impl_->chunks[i].rowType==type&&impl_->chunks[i].lapNumber<=(uint32_t)targetLap)candidates.push_back(i);std::sort(candidates.begin(),candidates.end(),[&](size_t a,size_t b){const auto&x=impl_->chunks[a];const auto&y=impl_->chunks[b];return x.lapNumber!=y.lapNumber?x.lapNumber>y.lapNumber:x.sequence>y.sequence;});
        for(size_t i:candidates){const auto&c=impl_->chunks[i];if(found&&c.lapNumber<foundLap)break;std::vector<V4TimedRow> rows;if(!impl_->splitChunk(i,rows,errorOut))return false;for(auto&r:rows)if(r.sessionTime<=at&&(!found||r.sessionTime>best.sessionTime||(r.sessionTime==best.sessionTime&&r.sequence>best.sequence))){best=std::move(r);found=true;foundLap=c.lapNumber;}}
        if(found)out.push_back(std::move(best));
    }sortRows(out);return true;
}
bool TnrdV4Archive::forEachChunk(V4RowTypeMask mask,const std::function<bool(const V4ChunkInfo&,std::string_view)>& callback,std::string* errorOut){std::vector<size_t> selected;for(size_t i=0;i<impl_->chunks.size();++i)if(mask&v4TypeBit((uint8_t)impl_->chunks[i].rowType))selected.push_back(i);std::sort(selected.begin(),selected.end(),[&](size_t a,size_t b){const auto&x=impl_->chunks[a];const auto&y=impl_->chunks[b];if(x.rowType!=y.rowType)return x.rowType<y.rowType;if(x.lapNumber!=y.lapNumber)return x.lapNumber<y.lapNumber;return x.sequence<y.sequence;});for(size_t i:selected){const auto&c=impl_->chunks[i];std::shared_ptr<std::string> plain;if(!impl_->loadChunk(i,plain,errorOut))return false;const std::string_view plainView(*plain);const size_t firstEnd=plainView.find('\n');if(scanTime(plainView.substr(0,firstEnd))>=0){if(!callback(c,*plain))return false;continue;}std::vector<V4TimedRow> rows;if(!impl_->splitChunk(i,rows,errorOut))return false;std::string normalized;normalized.reserve(plain->size()+rows.size()*24);for(const auto&row:rows){normalized+=withSessionTime(row.json,row.sessionTime);normalized.push_back('\n');}if(!callback(c,normalized))return false;}return true;}
void TnrdV4Archive::setCacheLimitBytes(size_t bytes){impl_->cacheLimit=bytes;while(impl_->cacheBytes>bytes&&!impl_->lru.empty()){const size_t old=impl_->lru.back();impl_->lru.pop_back();auto it=impl_->cache.find(old);impl_->cacheBytes-=it->second.plain->size();impl_->cache.erase(it);}}
size_t TnrdV4Archive::cacheBytes()const{return impl_->cacheBytes;}
uint64_t TnrdV4Archive::decompressedChunkCount()const{return impl_->decompressions;}

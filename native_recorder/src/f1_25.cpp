...
static nlohmann::json ParseStatusF125(const uint8_t* data, int base, const PacketHeader& hdr) {
    int o = base;
    o += 2;
...
std::vector<nlohmann::json> F1_25::ParsePacket(const uint8_t* data, int length, const PacketHeader& hdr, const std::string& timestamp) {
    ...
    switch (hdr.packetId) {
        ...
        case PID_CAR_STATUS: {
            ...
            nlohmann::json row = ParseStatusF125(data, base, hdr);
            row["type"] = "status";
            row["ts"] = timestamp;
            ...
            if (length >= HEADER_SIZE + 22 * statusSize) {
                nlohmann::json cars = nlohmann::json::array();
                for (int i = 0; i < 22; ++i) {
                    nlohmann::json cStat = ParseStatusF125(data, HEADER_SIZE + i * statusSize, hdr);
                    cStat["idx"] = i;

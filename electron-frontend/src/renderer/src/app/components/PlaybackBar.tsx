...
const ProgressTracker = memo(function ProgressTracker({ compact, currentTime, onSeekProgress, progressPct, totalTime }: { compact: boolean; currentTime: number; onSeekProgress: (progress: number) => void; progressPct: number; totalTime: number }) {
  ...
  return (
    <div className={`flex-1 flex items-center ${compact ? 'gap-2' : 'gap-4'}`}>
      ...
      <input ref={inputRef} type="range" min="0" max="1" step="0.001" defaultValue={progressPct} onMouseDown={() => { draggingRef.current = true }} onMouseUp={finishDrag} onTouchStart={() => { draggingRef.current = true }} onTouchEnd={finishDrag} onChange={handleChange} className="flex-1 h-1.5 bg-[var(--border)] rounded-full appearance-none outline-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[var(--color-info)] [&::-webkit-slider-thumb]:cursor-pointer" style={{ background: `linear-gradient(to right, var(--color-info) ${displayPct * 100}%, var(--border) ${displayPct * 100}%)` }} />
